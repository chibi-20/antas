...
<div class="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl shadow-sm p-6 mb-6 max-w-xl">
  <h2 class="text-sm font-semibold text-slate-600 dark:text-slate-300 mb-2">Bulk-create teacher accounts</h2>
  ...
  <form method="post">
    <?= csrf_field() ?>
    <label class="block text-sm font-medium text-slate-600 dark:text-slate-300 mb-1">Full names (one per line)</label>
    <textarea name="full_names" required rows="8" placeholder="Jay Mar V. Canturia&#10;Ireesh Joy J. Landicho" class="w-full mb-4 px-3 py-2 border border-slate-300 dark:border-slate-600 rounded-lg font-mono text-sm"></textarea>
    ...
    <a href="<?= h(url('/admin/users.php')) ?>" class="px-4 py-2 rounded-lg text-sm text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700">Back to Users</a>
  </form>
</div>

<?php if ($results && $results['created']): ?>
<div class="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl shadow-sm p-6 mb-6 max-w-xl">
  <h2 class="text-sm font-semibold text-slate-600 dark:text-slate-300 mb-3">Created (<?= count($results['created']) ?>)</h2>
  <p class="text-xs text-slate-400 dark:text-slate-500 mb-3">Default password for all of these: <strong>ilovejacobo</strong>. Copy the list below to hand out to teachers.</p>
    ...
    <table class="w-full text-sm">
      ...
      <tbody class="divide-y divide-slate-100 dark:divide-slate-700">
        ...
        <tr>
          ...
          <td class="px-4 py-2 font-mono text-slate-600 dark:text-slate-300"><?= h($c['username']) ?></td>
        </tr>
        <?php endforeach; ?>
      </tbody>
    </table>
  </div>
  <label class="block text-xs font-medium text-slate-500 dark:text-slate-400 mb-1">Copy for Excel/Sheets (Name, Username)</label>
  <textarea readonly rows="6" class="w-full px-3 py-2 border border-slate-300 dark:border-slate-600 rounded-lg font-mono text-xs bg-slate-50 dark:bg-slate-900" onclick="this.select()"><?= h(implode("\n", array_map(fn($c) => $c['full_name'] . "\t" . $c['username'], $results['created']))) ?></textarea>
</div>
<?php endif; ?>
