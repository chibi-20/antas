...
<div class="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl shadow-sm p-6 mb-6 max-w-lg">
  <h2 class="text-sm font-semibold text-slate-600 dark:text-slate-300 mb-4"><?= $editing ? 'Edit School Year' : 'Add School Year' ?></h2>
  <form method="post">
    <?= csrf_field() ?>
    <input type="hidden" name="action" value="<?= $editing ? 'update' : 'create' ?>">
    <?php if ($editing): ?><input type="hidden" name="id" value="<?= (int) $editing['id'] ?>"><?php endif; ?>
    <label class="block text-sm font-medium text-slate-600 dark:text-slate-300 mb-1">Year label (e.g. 2026-2027)</label>
    <input type="text" name="year_label" required value="<?= h($editing['year_label'] ?? '') ?>" class="w-full mb-4 px-3 py-2 border border-slate-300 dark:border-slate-600 rounded-lg">
    <div class="flex gap-2">
      ...
      <?php if ($editing): ?><a href="<?= h(url('/admin/school_years.php')) ?>" class="px-4 py-2 rounded-lg text-sm text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700">Cancel</a><?php endif; ?>
    </div>
  </form>
</div>
  ...
  <table class="w-full text-sm">
    ...
    <tbody class="divide-y divide-slate-100 dark:divide-slate-700">
      ...
      <tr>
        ...
        <td class="px-4 py-3"><?= $year['is_active'] ? '<span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-emerald-100 dark:bg-emerald-900/40 text-emerald-700 dark:text-emerald-300">Active</span>' : '<span class="text-slate-400 dark:text-slate-500">—</span>' ?></td>
        <td class="px-4 py-3 text-right space-x-2">
          <a href="<?= h(url('/admin/school_years.php?edit=' . $year['id'])) ?>" class="text-accent-600 dark:text-accent-400 hover:underline">Edit</a>
          <?php if (!$year['is_active']): ?>
          <form method="post" class="inline">
            <?= csrf_field() ?>
            <input type="hidden" name="action" value="set_active">
            <input type="hidden" name="id" value="<?= (int) $year['id'] ?>">
            <button type="submit" class="text-emerald-600 dark:text-emerald-400 hover:underline">Set Active</button>
          </form>
          <?php endif; ?>
        </td>
      </tr>
