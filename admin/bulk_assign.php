...
?>
<div class="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl shadow-sm p-6 mb-6 max-w-2xl">
  <h2 class="text-sm font-semibold text-slate-600 dark:text-slate-300 mb-2">Assign one teacher to many sections at once</h2>
  <p class="text-xs text-slate-400 dark:text-slate-500 mb-4">Pick a teacher and subject, then check every section they teach it in — including across different grade levels. Each checked section becomes its own assignment with submission tracking for all 3 terms, same as adding one at a time.</p>
  <form method="post">
    <?= csrf_field() ?>
    <div class="grid grid-cols-3 gap-3 mb-4">
      <div>
        <label class="block text-xs font-medium text-slate-500 dark:text-slate-400 mb-1">School year</label>
        <select name="school_year_id" onchange="this.form.submit()" required class="w-full px-3 py-2 border border-slate-300 dark:border-slate-600 rounded-lg">
          <?= select_options($schoolYears, 'id', 'year_label', $filterYearId) ?>
        </select>
      </div>
      <div>
        <label class="block text-xs font-medium text-slate-500 dark:text-slate-400 mb-1">Teacher</label>
        <select name="teacher_id" required class="w-full px-3 py-2 border border-slate-300 dark:border-slate-600 rounded-lg js-searchable" data-placeholder="Search teachers…">
      ...
      <div>
        ...
        <select name="subject_id" required class="w-full px-3 py-2 border border-slate-300 dark:border-slate-600 rounded-lg">
          <option value="">Select…</option>
    ...
    <label class="block text-sm font-medium text-slate-600 dark:text-slate-300 mb-2">Sections</label>
    <?php if (!$sectionsByGrade): ?>
      <p class="text-sm text-slate-400 dark:text-slate-500 mb-4">No active sections for this school year yet.</p>
    <?php endif; ?>
    <?php foreach ($sectionsByGrade as $grade => $secs): ?>
      <div class="mb-3">
        <div class="text-xs font-semibold text-slate-500 dark:text-slate-400 mb-1"><?= h($grade) ?></div>
        <div class="flex flex-wrap gap-3">
          <?php foreach ($secs as $sec): ?>
            <label class="inline-flex items-center gap-1.5 text-sm bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg px-3 py-1.5">
    ...
    <div class="flex gap-2 mt-4">
      ...
      <a href="<?= h(url('/admin/assignments.php')) ?>" class="px-4 py-2 rounded-lg text-sm text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700">Back to Assignments</a>
    </div>
  </form>
</div>

<?php if ($results && $results['skipped']): ?>
<div class="bg-white dark:bg-slate-800 border border-amber-200 dark:border-amber-800 rounded-xl shadow-sm p-6 max-w-2xl">
